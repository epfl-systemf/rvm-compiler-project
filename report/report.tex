...
format & \multicolumn{3}{l }{description} \\ \hline
\verb|mzero md| & \multicolumn{3}{l}{clear register \texttt{md}} \\ 
\verb|mld.w md, (rs1), rs2| & \multicolumn{3}{l }{load matrix from \texttt{rs1} into register \texttt{md}, with byte row stride \texttt{rs2}} \\ 
\verb|mst.w ms1, (rs1), rs2| & \multicolumn{3}{l}{store matrix in register \texttt{ms1} to \texttt{rs1}, with byte row stride \texttt{rs2}} \\
\end{tabular}
...
This issue is nuanced from the perspective of OptiTrust: recall that its goal is to validate the soundness of the schedule created by the user. At minimum, this requires some formalization of the program for the analyis carried out, in OptiTrust's case, being based on separation logic. But inline assembly is not a trivial construct to incorporate here, simply because the behavior is completely arbitrary: it depends on the hardware being compiled to! Exo's solution to this problem was for the user to just provide a usable specification for the instruction, in the language itself. OptiTrust is not a DSL though, so it remains to be seen what the best design solution is. This is an active area of research for the OptiTrust team.  

It's worth noting that in general, this optimization has been quite depth-first. 